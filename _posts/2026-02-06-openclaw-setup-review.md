...
*   **하이브리드 뇌:** 평소에는 압도적인 성능의 **Gemini 3 Flash**를 사용하다가, 할당량이 소진되거나 민감한 보안 정보를 다룰 때는 자동으로 로컬의 **Ollama**로 전환되는 지능형 폴백 시스템을 구축했습니다.
...
최근 OpenClaw를 포함한 AI 에이전트들의 보안 취약점에 대한 우려 섞인 소식들이 많이 들려오고 있습니다. 이에 루나와 저는 더욱 강력한 자체 보안 수칙을 세워 대응하고 있습니다.

*   **로컬 완결성 확보:** API 키, 개인 식별 정보 등 민감한 데이터는 클라우드 모델을 거치지 않고, 반드시 로컬 모델인 **Llama 3.2 3B**를 통해서만 처리하도록 설정했습니다.
*   **사전 승인 프로토콜:** 단순 파일 읽기를 제외한 모든 시스템 수정, 삭제, 실행 작업은 반드시 저의 사전 승인을 거쳐야만 루나가 동작하도록 프로그래밍했습니다.
*   **정기 건강검진:** 매주 토요일 오전 9시, 루나는 스스로 시스템 업데이트와 보안 감사를 수행하고 그 결과를 노션에 투명하게 기록합니다.
